...
def test_the_windows_adapter_reads_a_control_when_a_backend_is_present():
    class FakeControl:
        ControlTypeName, Name, AutomationId, ClassName, FrameworkId = "ButtonControl", "Search", "btnSearch", "Button", "WinForm"

    class FakeUia:
        ControlFromPoint = staticmethod(lambda x, y: FakeControl())

    detector = WindowsUiaDetector(backend=lambda: (FakeUia, ""))
    result = detector.observe(DiscoveryContext({"screen": {"x": 120, "y": 340}}))
...
def test_the_windows_adapter_reports_missing_when_no_control_is_under_the_point():
    class Empty:
        ControlFromPoint = staticmethod(lambda x, y: None)

    result = WindowsUiaDetector(backend=lambda: (Empty, "")).observe(DiscoveryContext({"screen": {"x": 1, "y": 1}}))
    assert result["status"] == fp.MISSING
