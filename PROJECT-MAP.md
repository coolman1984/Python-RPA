...
```
 user clicks an element in the selected Chrome tab
              │
              ...
              ▼
 probe.js  (smartops_desktop/probe.js, injected into every frame of the tab)
   gathers raw in-page evidence: web · frame · nexacro · anchor · relative · keyboard
   tags the element with data-smartops-probe=<token> so the worker can find the same node
              ...
              ▼
 worker.arm_radar  → pending[]           (smartops_desktop/worker.py)
   the binding ONLY queues; no Playwright call is ever made inside a Playwright callback
              │
              ▼
 worker.drain_radar  → DiscoveryManager.discover_element(payload, page, run_dir, index, frame)
              │
              ▼
 ...
 fingerprint.normalize  (smartops_desktop/fingerprint.py)
   fills every missing layer, bounds all page-supplied data, fixes the schema
              │
              ├─────────────► run folder:  runs/<run-id>/element-N.json
              │                            runs/<run-id>/element-N.png
              │                            runs/<run-id>/element-N-context.png
              │
...
| 5 | windows | `WindowsUiaDetector` | `uiautomation` on Windows | `layers.windows` | IMPLEMENTED_UNVERIFIED |
...
**Frame context is never ranked.** It says *where* to look, not *which* element, so
`fingerprint.rank()` excludes it and `score()["context"]` reports it separately.
...
| `tests/test_gui.py` | the radar card, the element list, tab guard | no |
