...
# Generated by the tool itself, so a change here is never the map falling behind the code.
GENERATED = {".project-eye/revision.json", ".project-eye/graph.yaml"}
...
def stale_proofs():
    ...
    if not base:
        return []
    changed = set(git("diff", "--name-only", f"{base}..HEAD").splitlines()) - GENERATED
    stale = []
